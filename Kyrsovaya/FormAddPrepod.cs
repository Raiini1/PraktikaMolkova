...
namespace Kyrsovaya
{
    public partial class FormAddPrepod : Form
    {
        ...
        {
            FormPrepodavatel formadd = (FormPrepodavatel)this.Owner;
            ...
            table = new DataTable();
            adapter.Fill(table);
            formadd.dataGridViewPrepod.DataSource = table;
            this.Close();
        }

        private void FormAddPrepod_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Вы точно хотите добавить данные в таблицу Преподаватели?", "Предупреждение", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (!string.IsNullOrEmpty(tbID.Text) && !string.IsNullOrWhiteSpace(tbID.Text) &&
               ...
               !string.IsNullOrEmpty(tb_doljnost.Text) && !string.IsNullOrWhiteSpace(tb_doljnost.Text) &&
               ...
               
            {
                using (var connection = new SqlConnection(get_cs()))
                {
                    ...
                    using (var cmd = new SqlCommand("INSERT INTO [Prepodavatel] (ID_Prepodavatel, Imya, Familia, Otchestvo, Doljnost, Data_rojdenia, Adres_projivania) VALUES (@ID, @Familia, @Imya, @Otchestvo, @Doljnost, @Data_rojdenia, @Adres_projivania)", connection))
